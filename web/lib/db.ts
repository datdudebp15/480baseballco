...
export async function audit(
  ...
  await db.run(
    "INSERT INTO audit_log (actor_email, action, detail, created_at) VALUES (?, ?, ?, ?)",
    [actorEmail, action, detail, isoNow()]
  );
}
...
async function seedProduction(db: Dbx) {
  ...
  await db.run(
    `INSERT INTO users (email, name, password_hash, role, is_member, waiver_accepted_at, member_since)
     VALUES (?, ?, ?, 'admin', 1, ?, ?)`,
    [email, "Warren Holzemer", bcrypt.hashSync(password, 10), isoNow(), isoNow()]
  );
  ...
  const addUser = async (
    ...
    (
      await db.get<{ id: number }>(
        `INSERT INTO users (email, name, password_hash, role, is_member, waiver_accepted_at, member_since)
         VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id`,
        [email, name, hash(pw), role, member, isoNow(), member ? isoNow() : null]
      )
    )!.id;
